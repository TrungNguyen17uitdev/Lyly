...
export const SkeletonItem: React.FC<SkeletonItemProps> = ({ container, style, ...props }) => {
  const progress = useSharedValue(0)

  useEffect(() => {
    if (container) return

    progress.value = withRepeat(withTiming(1, { duration: 1000, easing: Easing.linear }), 0, true)
  }, [container, progress])
